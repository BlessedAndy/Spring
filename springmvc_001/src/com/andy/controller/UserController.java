...
import com.andy.model.UserException;
...
public class UserController {
	...
	public UserController() {
		...
		users.put("wt",new User("wt","123","五图","asss"));
	}
	
	@RequestMapping(value="/users",method=RequestMethod.GET)//只有get请求才处理
	...
	public String add(@Validated User user,BindingResult br,@RequestParam("attachs")MultipartFile[] attachs,HttpServletRequest req) throws IOException {//一定要紧跟Validate之后写验证结果类
		...
		for(MultipartFile attach:attachs) {
			if(attach.isEmpty()) continue;  //如果没有上传文件就跳过
	...
	@ResponseBody
	public User show(@PathVariable String username) {//@PathVariable，路径里的值作为一个参数
	...
	public String login(String username,String password,HttpSession session) {
		...
		if(!u.getPassword().equals(password)) {
			throw new UserException("用户密不正确");
		}
